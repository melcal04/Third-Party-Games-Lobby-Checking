...
export class SAGamingPage extends PageHelper {
  ...
  async extractLobbyData(): Promise<Record<string, any>> {
    ...
    await this.page.getByRole("link").filter({ hasText: "Live Games" }).click({ timeout: 5000 });
    await this.page.waitForTimeout(3000);
    const categorySelector = "div.swiper-backface-hidden div.swiper-wrapper button:not(:text('All Games'))";
  ...
  async getTables(): Promise<string[]> {
    const tableNames: Locator = this.page.locator("div.css-1ar92i8 div.header-title-container:first-of-type");
